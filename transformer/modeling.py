# coding=utf-8
# code obtained from huggingface transformers
"""PyTorch BERT model."""
import os
import copy
import json
...
# from .cons_att import get_constrained_attention_scores, Entropy
...
#         print(query_layer.size(), key_layer.size(), attention_scores.size(), attention_mask.size())
        if self.seg_emb is not None and False:
        ...
        return context_layer_2, selective_att_scores, selective_att_scores_r
...
class BertAttention(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask, history_states=None, mask_qkv=None, seg_ids=None, att_ids=None):
        self_output, selective_att_scores, selective_att_scores_r = self.self(
            input_tensor, attention_mask, history_states=history_states, mask_qkv=mask_qkv, seg_ids=seg_ids, att_ids=att_ids)
        attention_output = self.output(self_output, input_tensor)
        return attention_output, selective_att_scores, selective_att_scores_r
...
class BertLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, history_states=None, mask_qkv=None, seg_ids=None, att_ids=None):
        attention_output, selective_att_scores, selective_att_scores_r = self.attention(
        ...
        return layer_output, selective_att_scores, selective_att_scores_r
    ...
    def forward(self, hidden_states=None, attention_mask=None, output_all_encoded_layers=True, prev_embedding=None, prev_encoded_layers=None, mask_qkv=None, seg_ids=None, att_ids=None):
        ...
        if (prev_embedding is not None) and (prev_encoded_layers is not None):
            ...
            for i, layer_module in enumerate(self.layer):
                hidden_states, selective_att_scores, selective_att_scores_r = layer_module(
                    hidden_states, attention_mask, history_states=history_states, mask_qkv=mask_qkv, seg_ids=seg_ids, att_ids=att_ids)
        ...
        return all_encoder_layers, sum_selective_att_scores, sum_selective_att_scores_r
...
class BertModel(PreTrainedBertModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, output_all_encoded_layers=True, mask_qkv=None, task_idx=None, match_index=None):
        ...
        pooled_output = self.pooler(sequence_output, match_index)
        if not output_all_encoded_layers:
...
class BertVideoEncoder(nn.Module):
    ...
    def forward(self, video_features, token_type_ids, position_ids, video_mask):
        ...
        video_embeddings = self.embeddings.LayerNorm(video_embeddings)
        
        video_features = self.encoder(video_embeddings, video_mask)[0]
...
class BertTextEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.embeddings = BertEmbeddings(config)
        self.new_text_embeddings = nn.Sequential(
            nn.Linear(config.hidden_size, config.hidden_size),
            nn.Linear(config.hidden_size, config.hidden_size)
        )
    def forward(self, input_ids, token_type_ids, position_ids, text_mask=None):
        embedding_output = self.embeddings(input_ids, token_type_ids, position_ids)
        embedding_output = self.new_text_embeddings(embedding_output)
...
class BertModelIncr(BertModel):
    ...
    def forward(self, inputs=None, attention_mask=None, token_type_ids=None, output_all_encoded_layers=True, prev_embedding=None, prev_encoded_layers=None, mask_qkv=None, task_idx=None, att_ids=None):
        ...
        encoded_layers, att_output, att_output_r = self.encoder(inputs,
                                      attention_mask,
        ...
        if not output_all_encoded_layers:
            encoded_layers = encoded_layers[-1]
        return sequence_output, encoded_layers, pooled_output, att_output, att_output_r
...
class BertForSeq2SeqDecoder(PreTrainedBertModel):
    ...
    def __init__(self, config, max_v_len=64, mask_word_id=103, num_rel=0,
        ...
        self.video_encoder = BertVideoEncoder(config)
        self.text_encoder = BertTextEncoder(config)

        self.loss = LabelSmoothingLoss(0.1, config.vocab_size, ignore_index=0)    
    ...
    def forward(self, video_features, video_mask, input_ids, attention_mask, token_type_ids, position_ids, text_labels, loss_mask, match_labels, task_idx=None, mask_qkv=None, loss_masks=None, att_ids=None):       

        extended_attention_mask = self.bert.get_extended_attention_mask(attention_mask)
        video_features = self.video_encoder(video_features, token_type_ids[:, :self.max_v_len], position_ids[:, :self.max_v_len], self.bert.get_extended_attention_mask(video_mask))   
        text_features = self.text_encoder(input_ids, token_type_ids[:, self.max_v_len:], position_ids[:, self.max_v_len:], self.bert.get_extended_attention_mask(attention_mask[:, self.max_v_len:, self.max_v_len:])) 
        
        inputs = torch.cat([video_features, text_features], 1)  
               
        sequence_output, encoded_layers, pooled_output, att_output, att_output_r = self.bert(
            inputs, extended_attention_mask, token_type_ids, output_all_encoded_layers=False, mask_qkv=mask_qkv, task_idx=task_idx, att_ids=att_ids)
        
        if self.use_att:
            entropy = Entropy() 
            selective_att_loss = 0.5 * entropy(torch.stack(att_output_r, 1), att_ids[:, 4:5]) + 0.5 * entropy(torch.stack(att_output, 1), att_ids[:, 4:5])
        else:
            selective_att_loss = torch.zeros(1)
        def loss_mask_and_normalize(loss, mask):
            mask = mask.type_as(loss)
            loss = loss * mask
            denominator = torch.sum(mask) + 1e-12
            return (loss / denominator).sum()
        
        sequence_output = sequence_output[:, self.max_v_len:]
        prediction_scores, seq_relationship_score = self.cls(
            sequence_output, pooled_output, task_idx=task_idx)
        
        match_scores = seq_relationship_score.argmax(1)
        match_loss = F.binary_cross_entropy_with_logits(seq_relationship_score, match_labels*1.0).mean()
        
        masked_loss = self.loss(prediction_scores.contiguous().view(-1, prediction_scores.size(-1))*1.0, text_labels.view(-1))
        masked_loss = loss_mask_and_normalize(
            masked_loss*1.0, loss_mask.contiguous().view(-1))
        
        return masked_loss.mean(), match_loss, selective_att_loss.mean(), prediction_scores, match_scores
